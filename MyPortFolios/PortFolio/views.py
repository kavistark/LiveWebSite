...
def folder(request, folderid):
    if request.user.is_authenticated:
        ...
        context = {'folderid': folderid, 'files': files, 'student': student}
        ...
        if request.method == 'POST':
            ...
            fileadd = File.objects.create(filetitle=file_title, file=file_user, folder=folder_user, upload_date=upload_date)
...
def folder_views(request):
    ...
    context = {'folder': folder, 'student': student, 'user': request.user,
                       'generated_link': generated_link}
    return render(request, 'folder_views.html', context)

def approve_file(request, file_id):
    file_instance = get_object_or_404(File, id=file_id)
    file_instance.approved = True
    file_instance.declined = False 
    file_instance.save()
    return redirect('approved_files')

def decline_file(request, file_id):
    file_instance = get_object_or_404(File, id=file_id)
    file_instance.declined = True
    file_instance.approved = False
    file_instance.save()
    return redirect('student_details', student_id=file_instance.folder.folderuser.student.id)

def approved_files(request):
    approved_files = File.objects.filter(approved=True).select_related('folder__folderuser')
    return render(request, 'approved_files.html', {'approved_files': approved_files})
    
def approve_or_decline_file(request, file_id):
    file_instance = get_object_or_404(File, id=file_id)
